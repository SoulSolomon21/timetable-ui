import { BookOpen, Check, GraduationCap, MapPin, Search, SlidersHorizontal, X } from 'lucide-react';
...
// ─── Stepped Mode ─────────────────────────────────────────────────────────────
function SteppedMode({ data, flatOptions, onSelect, selected }) {
  const [campus, setCampus] = useState('')
  const [faculty, setFaculty] = useState('')
  const [dept, setDept] = useState('')

  const campuses = useMemo(() => data.map(c => c.campusName), [data])
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Step 1 — Campus */}
      <StepWrapper number={1} label="Campus" active={true} done={!!campus}>
      ...
      <StepWrapper number={2} label="Faculty / School" active={!!campus} done={!!faculty}>
        <Select value={faculty} onValueChange={handleFacultyChange} disabled={!campus}>
          <SelectTrigger
            className={cn(
              'w-full h-10 text-sm border-stone-200 focus:ring-teal-500 focus:border-teal-400',
              !campus && 'opacity-50 cursor-not-allowed',
            )}
          >
      ...
      <StepWrapper number={3} label="Department" active={!!faculty} done={!!dept}>
        <Select value={dept} onValueChange={handleDeptChange} disabled={!faculty}>
          <SelectTrigger
            className={cn(
              'w-full h-10 text-sm border-stone-200 focus:ring-teal-500 focus:border-teal-400',
              !faculty && 'opacity-50 cursor-not-allowed',
            )}
          >
...
function StepWrapper({ number, label, active, done, children }) {
  return (
    <div className={cn('flex items-start gap-3 transition-opacity', !active && 'opacity-40')}>
      <div
        className={cn(
...
// ─── Selection Preview ────────────────────────────────────────────────────────
function SelectionPreview({ selected, onClear }) {
  if (!selected)
    return null
  return (
...
export default function ProgramPicker() {
  ...
  const flatOptions = useMemo(() => flattenData(RAW_DATA), [])
